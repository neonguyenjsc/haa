...
class FacebookViewStoryController extends Controller
{
    //


    protected $menu_id = 53;
    ...
    public function index()
    {
        ...
        return view('Ads.Facebook.ViewStory.index', ['menu' => $menu, 'package' => $package]);
    }
    ...
    public function history(Request $request)
    {
        ...
        return view('Ads.Facebook.ViewStory.history', ['data' => $data, 'menu' => $menu]);
    }
            ...
            if (!isset($checkMinMax['error'])) {
                switch ($package_name) {
                    case 'facebook_view_story':
                        ...
                        $params = \request()->all();
                        $params['quantity'] = abs(intval($params['quantity']));
                        $params['object_id'] = getUrlReplaceString($params['object_id'], "?");
                        $form_data = [
                            'gc' => $params['note'] ?? '',
                            'gtmtt' => $pricesMaster,
                            'lhi' => '',
                            'lsct' => '0',
                            'slct' => $params['quantity'],
                            'type_api' => 'buffviewstory',
                            'url' => ''
                        ];
                        preg_match('/(.*)www.facebook.com\/stories\/([0-9]{8,})\/(.*)/m', $params['object_id'], $link_data);
                        if (isset($link_data[2]) && isset($link_data[3])) {
                            $form_data['lhi'] = $link_data[2];
                            $form_data['url'] = $link_data[3];
                        ...
                        break;
                    case 'facebook_view_story_v3':
                        preg_match('/(.*)www.facebook.com\/stories\/([0-9]{8,})\/(.*)/m', $request->object_id, $link_data);
                        if (isset($link_data[2]) && isset($link_data[3])) {
                            $check_out_coin = $quantity * $pricesMin->prices;
                            $checkCoinAndHandleCoin = $this->coinSerivce->checkCoinAndHandleCoin($user->id, $check_out_coin);
                            if (!isset($checkCoinAndHandleCoin['error'])) {
                                $url = DOMAIN_TRAODOISUB . '/mua/viewstr/themid.php';
                        ...
                        return ['error_' => 'Link không đúng định dạng'];
                        break;
                    case 'facebook_view_story_v2':
                        $check_out_coin = $quantity * $pricesMin->prices;
                        $checkCoinAndHandleCoin = $this->coinSerivce->checkCoinAndHandleCoin($user->id, $check_out_coin);
                        if (!isset($checkCoinAndHandleCoin['error'])) {
                            $ads = $request->all();
