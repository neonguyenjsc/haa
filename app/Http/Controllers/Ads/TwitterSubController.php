...
class TwitterSubController extends Controller
{
    //
    protected $menu_id = 114;
...
//        return view('Ads.Twitter.Comment.index', ['menu' => $menu, 'package' => $package]);
//    }
//
//    public function history(Request $request)
//    {
//        $menu = Menu::find($this->menu_id);
//        $data = $this->getHistory('twitter', $this->menu_id, $request);
//        return view('Ads.Twitter.Comment.history', ['data' => $data, 'menu' => $menu]);
//    }
    ...
    {
        $log = Twitter::where('id', $id)->where('menu_id', $this->menu_id)->whereIn('status', [0, 1, 5, -1])->where(function ($q) use ($user) {
            if ($user->role != 'admin') {
        ...
        if (in_array($log->package_name, $log->dvo)) {
            ...
            if ($item && !isset($response->error)) {
                ...
                if ($response->status == 'Canceled' && $item->status != 0) {
                    if (!Refund::where('orders_id', $log->id)->where('table', 'twitter')->first()) {
                        $log->status = 0;
                        $log->save();
                        Logs::newLogs([
                            'user_id' => $log->user_id,
        ...
        if (in_array($log->package_name, $log->mfb)) {
            ...
            $url = DOMAIN_MFB . '/api/advertising/Twitter/list?limit=50&orders_id=' . $log->orders_id;
