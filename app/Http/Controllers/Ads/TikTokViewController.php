...
class TikTokViewController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('Ads.TikTok.View.index', ['menu' => $menu, 'package' => $package]);
    }
    ...
    public function history(Request $request)
    {
        ...
        return view('Ads.TikTok.View.history', ['data' => $data, 'menu' => $menu]);
    }
    ...
    public function actionBuy($request, $user)
    {
        ...
        if (!isset($pricesData['error'])) {
            ...
            if (!isset($checkMinMax['error'])) {
                switch ($package_name) {
                    case 'tiktok_view_s10':
                    case 'tiktok_view_s11':
                    case 'tiktok_view_s12':
                    case 'tiktok_view_s15':
                        $regex = "/(https:\/\/www\.tiktok\.com\/@)(.*)(\/)([0-9]{0,20})/";
                        preg_match($regex, $object_id, $data_regex);
                        if (count($data_regex) < 5) {
                            return ['error_' => 'link có định dạng https://www.tiktok.com/@username/video/7104254658092322090'];
                        }
                        if (($quantity % 1000) != 0) {
                            return ['error_' => 'Vui lòng mua số lượng chẳn. 1000,2000, 50000,.... (Bội số của 10000)'];
                        }
                        ...
                        if (!isset($checkCoinAndHandleCoin['error'])) {
                            $post_data = [
                                'api' => 'c42f628270729cc459eaab5f79c71e7d',
                                'link' => $object_id,
                                'number' => $quantity,
                                'sv' => $prices->package_name_master,
                            ];
                            $url = DOMAIN_MLIKE . '/api/order.php?service=view_tiktok&act=buy';
                            $response = $this->mlikeService->buy($url, $post_data);
                            if ($response && $response->status == "success") {
                                $ads = $request->all();
                                $ads['orders_id'] = $response->id_order ?? 0;
                                $ads['user_id'] = $user->id;
                        ...
                        break;
                    case 'tiktok_view_s13':
                    case 'tiktok_view_s5':
                    case 'tiktok_view_s14':
                    case 'tiktok_view_s16':
                        if (($quantity % 100) != 0) {
