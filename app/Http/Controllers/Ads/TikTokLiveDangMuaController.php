...
class TikTokLiveDangMuaController extends Controller
{
    //
    protected $menu_id = 90;
...
//    }
    ...
    public function actionBuy($request, $user)
    {
        ...
        if (!isset($pricesData['error'])) {
            ...
            if (!isset($checkMinMax['error'])) {
                switch ($package_name) {
                    default:
                        $regex = '/(tiktok\.com\/@.*\/live)/';
                        preg_match($regex, $request->object_id, $data_regex);
                        if (!isset($data_regex[0])) {
                            return ['error_' => 'Link chuẩn https://www.tiktok.com/@baostar/live'];
                        }
    ...
    public function actionCheckOrder($request, $user, $id)
    {
        ...
        if (in_array($log->package_name, $log->muaviewvn)) {
            $data = [
                'orders' => $log->orders_id,
                'action' => 'status',
            ];
            $response = $this->muaViewVnService->checkOrder($data);
            if ($item && !isset($response->error)) {
                $o = $log->orders_id;
                $response = $response->$o;
                $log->start_like = intval($response->start_count) ?? 0;
