...
class TelegramPostViewController extends Controller
{
    //

    protected $menu_id = 121;
    ...
    public function index()
    {
        ...
        return view('Ads.Telegram.PostView.index', ['menu' => $menu, 'package' => $package]);
    }

    public function history(Request $request)
    {
        $menu = Menu::find($this->menu_id);
        $data = $this->getHistory('telegram', $this->menu_id, $request);
        return view('Ads.Telegram.PostView.history', ['data' => $data, 'menu' => $menu]);
    }
    ...
    public function actionBuy($request, $user)
    {
        ...
        if (!isset($pricesData['error'])) {
            ...
            if (!isset($checkMinMax['error'])) {
                switch ($package_name) {
                    case 'telegram_mem_sv2':
                    case 'telegram_mem_sv3':
                    case 'telegram_mem_sv4':
    ...
    public function actionCheckOrder($request, $user, $id)
    {
        ...
        if (in_array($log->package_name, $log->dvo)) {
            ...
            if ($item && !isset($response->error)) {
                ...
                if ($response->status == 'Canceled' && $item->status != 0) {
                    if (!Refund::where('orders_id', $log->id)->where('table', 'telegram')->first()) {
                        $log->status = 0;
                        $log->save();
                        Logs::newLogs([
                            'user_id' => $log->user_id,
        ...
        if (in_array($log->package_name, $log->mfb)) {
            ...
            $url = DOMAIN_MFB . '/api/advertising/telegram/list?limit=50&orders_id=' . $log->orders_id;
