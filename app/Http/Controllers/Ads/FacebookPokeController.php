...
class FacebookPokeController extends Controller
{
    //
    protected $menu_id = 51;
    ...
    public function index()
    {
        ...
        return view('Ads.Facebook.Poke.index', ['menu' => $menu, 'package' => $package]);
    }
    ...
    public function history(Request $request)
    {
        ...
        return view('Ads.Facebook.Poke.history', ['data' => $data, 'menu' => $menu]);
    }
...
//            'quantity' => 'integer|min:1',
        ...
        if (!isset($pricesData['error'])) {
            ...
            $url = DOMAIN_AUTOFB_PRO . '/api/fbchocbanbe/createbot';
    ...
    public function apiCheckCookie(Request $request)
    {

//        return $this->returnActionApi($this->checkCookie($request->all()));
        $check = $this->checkCookie($request->all());
        if (is_array($check) && isset($check['error'])) {
            $this->res['status'] = 400;
            $this->res['success'] = false;
            $this->res['message'] = "Cookie không hợp lệ";
        } else {
            $this->res['data'] = $check->data;
        }
        return $this->setResponse($this->res);
    }
