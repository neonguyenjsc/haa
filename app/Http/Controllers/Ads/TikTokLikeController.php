...
class TikTokLikeController extends Controller
{
    //

    protected $menu_id = 81;
    ...
    public function index()
    {
        ...
        return view('Ads.TikTok.Like.index', ['menu' => $menu, 'package' => $package]);
    }
    ...
    public function history(Request $request)
    {
        ...
        return view('Ads.TikTok.Like.history', ['data' => $data, 'menu' => $menu]);
    }
                ...
                switch ($package_name) {
                    ...
                    case 'tiktok_like_v11':
                        ...
                        if (!isset($checkCoinAndHandle['error'])) {
                            ...
                            $url = DOMAIN_MLIKE . '/api/buy/tiktok/like_v2.php';
                            $response = $this->mlikeV2Service->callApi($url, $form_data);
                            if ($response && $response->status == "success") {
                                $ads = $request->all();
                                $ads['user_id'] = $user->id;
                                $ads['orders_id'] = $response->id_order ?? '';
                                $ads['username'] = $user->username;
                                $ads['link'] = $link;
                                $ads['package_name'] = $prices->package_name;
                                $ads['prices'] = $check_out_coin;
                    ...
                    case 'tiktok_like_v12':
                        ...
                        return returnDataError($checkCoinAndHandleCoin);
                        break;
                    case 'tiktok_like_v13':
                        $check_out_coin = $quantity * $pricesMin->prices;
                        $checkCoinAndHandleCoin = $this->coinSerivce->checkCoinAndHandleCoin($user->id, $check_out_coin);
                        if (!isset($checkCoinAndHandleCoin['error'])) {
                            $url = DOMAIN_TRAODOISUB . '/mua/tiktok_like/themid.php';
                            $time = time() . '_' . str_rand(3);
                            $post_data = [
                                'maghinho' => $time,
                                'id' => $request->object_id,
                    ...
                    case 'tiktok_like_v14':
                        ...
                        break;
                    default:
                        $pricesMaster = $this->mfbService->getPricesMaster(88, 'tiktok_like');
                        if ($pricesMaster) {
                            $check_out_coin = $quantity * $pricesMin->prices;
                            $checkCoinAndHandleCoin = $this->coinSerivce->checkCoinAndHandleCoin($user->id, $check_out_coin);
                            if (!isset($checkCoinAndHandleCoin['error'])) {
