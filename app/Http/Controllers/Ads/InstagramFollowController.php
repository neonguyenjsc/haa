...
class InstagramFollowController extends Controller
{
    //


    protected $menu_id = 72;
    ...
    public function index()
    {
        ...
        return view('Ads.Instagram.Follow.index', ['menu' => $menu, 'package' => $package]);
    }
    ...
    public function history(Request $request)
    {
        ...
        return view('Ads.Instagram.Follow.history', ['data' => $data, 'menu' => $menu]);
    }
            ...
            if (!isset($checkMinMax['error'])) {
                switch ($package_name) {
                    ...
                    case 'instagram_follow_sv2':
                        ...
                        if ($pricesMaster) {
                            ...
                            if (!isset($checkCoinAndHandleCoin['error'])) {
                                ...
                                $post_data['link'] = 'https://www.instagram.com/' . $object_id;
                                ...
                                if (isset($response->status) && $response->status == 200) {
                                    $ads = $request->all();
                                    $data = $response->data;
                                    $ads['orders_id'] = $data->orders_id ?? 0;
                                    $ads['user_id'] = $user->id;
                                    $ads['username'] = $user->username;
                        ...
                        break;
                    case 'instagram_follow_sv4':
                    case 'instagram_follow_sv7':
                    case 'instagram_follow_sv8':
                    case 'instagram_follow_sv9':
                        ...
                        break;
                        break;
                    case 'instagram_follow_sv10':
                        $regex = '/instagram.com/';
                        preg_match($regex, $request->object_id, $data_regex);

                        if (!isset($data_regex[0])) {
                            return ['error_' => 'Vui lòng nhập link'];
                        }
                        ...
                        if ($pricesMaster) {
                            ...
                            if (!isset($checkCoinAndHandleCoin['error'])) {
                                $post_data = [
                                    ...
                                    "object_id" => $this->convertUIDIg($request->object_id, "follow"),
                                    "quantity" => $quantity,
    ...
    public function actionCheckOrder($request, $user, $id)
    {
        ...
        if (in_array($log->package_name, $log->mfb)) {
            ...
            $url = DOMAIN_MFB . '/api/instagram-ads/list?limit=50&orders_id=' . $log->orders_id;
