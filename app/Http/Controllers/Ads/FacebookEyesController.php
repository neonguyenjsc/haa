...
class FacebookEyesController extends Controller
{
    //

    protected $menu_id = 49;
        ...
        if (is_int($sl->amount_livestream_unit_limited_available)) {
            foreach ($package as $item) {
                switch ($item->package_name) {
                    case 'facebook_eyes':
                        $item->config_package = "Số mắt khả dụng : " . number_format($this->buffViewerService->getSL()->amount_livestream_unit_limited_available ?? 0);
                        break;
//                case '45_3':
//                    $item->sl = "Số mắt khả dụng : " . number_format($this->getSLViewAdBreak()->viewers ?? 0);
//                    $item->sl = "";
//                    break;
//                case '45_4':
//                    $item->sl = "";
//                    break;
        ...
        return view('Ads.Facebook.Eyes.index', ['menu' => $menu, 'package' => $package]);
    }

    public function history(Request $request)
    {
        $menu = Menu::find($this->menu_id);
        $data = $this->getHistory('facebook_eyes', $this->menu_id, $request);
        return view('Ads.Facebook.Eyes.history', ['data' => $data, 'menu' => $menu]);
    }
    ...
    public function actionBuy($request, $user)
    {
        $validate = Validator::make($request->all(), [
            ...
            'num_minutes' => 'required|integer|min:30',
        ], [
        ...
        $num_minutes = abs(intval($request->num_minutes));
        ...
        if (!isset($pricesData['error'])) {
            ...
            if (!isset($checkMinMax['error'])) {
                switch ($package_name) {
                    case 'facebook_eyes_sv1':
                    case 'facebook_eyes_sv2':
                        $params = \request()->all();
                        ...
                        $form_data = [
                            ...
                            "lvctm" => '',
                            "slmct" => abs(intval($params['quantity'])),
                            "tgdtm" => abs(intval($params['num_minutes'])),
                            "type_api" => 'Buff_view_livestrean',
                        ];
//                        preg_match('/(.*)www.facebook.com\/(.*)\/videos\/(.*)/m', $params['object_id'], $link_data);
//                        if (!isset($link_data[3])) {
//                            return ['error_' => 'Đường dẫn không hợp lệ'];
//                        }
                        $form_data['lvctm'] = $object_id ?? '';
                        $url = DOMAIN_AUTOFB_PRO . '/api/facebook_buff/create';
                        $check_out_coin = $quantity * $pricesMin->prices * $num_minutes;
                    ...
                    default:
                        ...
                        if (!isset($checkCoinAndHandleCoin['error'])) {
                            ...
                            $data = array();
                            $data[0]['name'] = $user->username;
                            $data[0]['post_id'] = $object_id;
                            $data[0]['amount'] = abs(intval($request->get('quantity')));
                            $data[0]['num_minutes'] = abs(intval($request->get('num_minutes')));
                            $data[0]['note'] = $request->get('notes');
                            $data[0]['is_unlimited'] = ($request->get('sever') == 2) ? true : false;
                            $data[0]['server_id'] = $prices->package_name_master;
