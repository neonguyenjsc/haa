...
class YoutubeViewController extends Controller
{
    //

    protected $menu_id = 93;
    ...
    public function index()
    {
        ...
        return view('Ads.Youtube.View.index', ['menu' => $menu, 'package' => $package]);
    }
    ...
    public function history(Request $request)
    {
        ...
        return view('Ads.Youtube.View.history', ['data' => $data, 'menu' => $menu]);
    }
    ...
    public function actionBuy($request, $user)
    {
        ...
        if (!isset($pricesData['error'])) {
            ...
            if (!isset($checkMinMax['error'])) {
                switch ($package_name) {
//                    case 'youtube_view_good123123':
                    case 'youtube_view_sv1':
                    case 'youtube_view_sv2':
                    case 'youtube_view_sv12':
                    case 'youtube_view_sv13':
                    case 'youtube_view_sv14':
                        ...
                        break;
                    case 'youtube_time_view_4k_sv21':
//                    case 'youtube_view_release':
                        $check_out_coin = $quantity * $pricesMin->prices;
                        $checkCoinAndHandleCoin = $this->coinSerivce->checkCoinAndHandleCoin($user->id, $check_out_coin);
                        if (!isset($checkCoinAndHandleCoin['error'])) {
                            $data = $request->all();
                        ...
                        return returnDataError($checkCoinAndHandleCoin);
                        break;
//                    case 'youtube_view_8p_20p':
//                    case 'youtube_view_5p_20p':
                    case 'youtube_time_view_4k_':
//                    case 'youtube_time_view_4k_sv2':
                        $check_out_coin = $quantity * $pricesMin->prices;
                        $checkCoinAndHandleCoin = $this->coinSerivce->checkCoinAndHandleCoin($user->id, $check_out_coin);
                        if (!isset($checkCoinAndHandleCoin['error'])) {
                            $data = $request->all();
                            $data['action'] = 'add';
                            $data['service'] = $prices->package_name_master;
                            $response = $this->vnKingService->callApi($data);
                            if ($response && isset($response->order)) {
                                $ads = $request->all();
                                $ads['orders_id'] = $response->order ?? 0;
                                $ads['user_id'] = $user->id;
