...
class TikTokFollowController extends Controller
{

    protected $menu_id = 82;
    ...
    public function index()
    {
        ...
        return view('Ads.TikTok.Follow.index', ['menu' => $menu, 'package' => $package]);
    }
    ...
    public function history(Request $request)
    {
        ...
        return view('Ads.TikTok.Follow.history', ['data' => $data, 'menu' => $menu]);
    }
    ...
    public function actionBuy($request, $user)
    {
        ...
        if (!isset($pricesData['error'])) {
            ...
            if (!isset($checkMinMax['error'])) {
                switch ($package_name) {
                    case 'tiktok_follow_sv3':
                    case 'tiktok_follow_sv4':
                    case 'tiktok_follow_sv5':
                    case 'tiktok_follow_sv2':
                        ...
                        break;
                    case 'tiktok_follow_sv6':
                    case 'tiktok_follow_sv7':
                        $check_out_coin = $quantity * $pricesMin->prices;
                        $checkCoinAndHandleCoin = $this->coinSerivce->checkCoinAndHandleCoin($user->id, $check_out_coin);
                        if (!isset($checkCoinAndHandleCoin['error'])) {
                            $ads = $request->all();
                            $ads['user_id'] = $user->id;
                            $ads['username'] = $user->username;
                            $ads['link'] = 'https://tiktok.com/' . $object_id;
                            $ads['package_name'] = $prices->package_name;
                            $ads['prices'] = $check_out_coin;
                            $ads['price_per'] = $pricesMin->prices;
                            $ads['quantity'] = $quantity;
                        ...
                        return returnDataError($checkCoinAndHandleCoin);
                        break;
                    case 'tiktok_follow_sv8_':
                        $pricesMaster = $this->mfbService->getPricesMaster(88, $prices->package_name_master);
                        if ($pricesMaster) {
                            $check_out_coin = $quantity * $pricesMin->prices;
                            $checkCoinAndHandleCoin = $this->coinSerivce->checkCoinAndHandleCoin($user->id, $check_out_coin);
                            if (!isset($checkCoinAndHandleCoin['error'])) {
                                ...
                                $post_data = [
                                    ...
                                    "object_id" => $this->convertUidTikTok($object_id),
                                    "quantity" => $quantity,
...
//                                $post_data['link'] = $link;
                                $post_data['provider'] = 'tiktok';
                                $url = DOMAIN_MFB . '/api/tiktok-ads/create';
                    ...
                    case 'tiktok_follow_sv10':
                        ...
                        if (!isset($checkCoinAndHandle['error'])) {
                            ...
                            $form_data = [
                                'link' => $this->convertUidTikTokV2($request->object_id, 'follow'),
                                'sl' => $quantity,
                                'maghinho' => $time,
                                'magiamgia' => null,
                                'dateTime' => urlencode(date('Y-m-d H:i:s')),
                            ];
                            $url = 'https://tuongtaccheo.com/tiktok/tangsub/themvip.php';
                        ...
                        break;
                    case 'tiktok_follow_sv13':
                        $check_out_coin = $quantity * $pricesMin->prices;
                        $checkCoinAndHandleCoin = $this->coinSerivce->checkCoinAndHandleCoin($user->id, $check_out_coin);
                        if (!isset($checkCoinAndHandleCoin['error'])) {
                            $url = DOMAIN_TRAODOISUB . '/mua/tiktok_follow/themid.php';
                            $time = time() . '_' . str_rand(3);
                            $post_data = [
                                'maghinho' => $time,
                        ...
                        break;
                    case 'tiktok_follow_sv12':
                    case 'tiktok_follow_sv11':
                        $check_out_coin = $quantity * $pricesMin->prices;
                        $checkCoinAndHandleCoin = $this->coinSerivce->checkCoinAndHandleCoin($user->id, $check_out_coin);
                        if (!isset($checkCoinAndHandleCoin['error'])) {
                            $data = $request->all();
                        ...
                        break;
                        break;
                    case 'tiktok_follow_sv14':
                    case 'tiktok_follow_sv15':
                        $check_out_coin = $quantity * $pricesMin->prices;
                        $checkCoinAndHandleCoin = $this->coinSerivce->checkCoinAndHandleCoin($user->id, $check_out_coin);
                        if (!isset($checkCoinAndHandleCoin['error'])) {
                            $data = $request->all();
                        ...
                        break;
                    default:
                        $pricesMaster = $this->mfbService->getPricesMaster(88, 'tiktok_follow');
                        if ($pricesMaster) {
                            $check_out_coin = $quantity * $pricesMin->prices;
                            $checkCoinAndHandleCoin = $this->coinSerivce->checkCoinAndHandleCoin($user->id, $check_out_coin);
                            if (!isset($checkCoinAndHandleCoin['error'])) {
...
//                                $post_data['link'] = $link;
    ...
    public function actionCheckOrder($request, $user, $id)
    {
        ...
        if (in_array($log->package_name, $log->mfb)) {
            ...
            $url = DOMAIN_MFB . '/api/advertising/tiktok/list?limit=50&orders_id=' . $log->orders_id;
...
//                    $log->start_like = $item->start ?? 0;
    ...
    public function convertUidTikTokV2($link, $type)
    {
        if (strpos($link, "?") > -1) {
