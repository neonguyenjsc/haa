...
class AutoUpdateCountIsRunInstagramMfb extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'update_count_is_run_instagram';
    protected $limit = 500;
    /**
 ...
 | |_| | |_) | (_| | (_| | ||  __/  | (_| (_) | |_| | | | | |_   | \__ \  | |  | |_| | | | |
  \__,_| .__/ \__,_|\__,_|\__\___|   \___\___/ \__,_|_| |_|\__|  |_|___/  |_|   \__,_|_| |_|
       |_|                                                                                  
\n";
        $keyCacheAutoUpdateCountIsRun = build_key_cache(['keyCacheAutoUpdateCountIsRunIg']);
...
//                ],
                'instagram' => [
                    'url' => 'https://api.mfb.vn/api/advertising/instagram/list?limit=' . $this->limit,
                    'provider' => 'instagram',
                ],
            ];
            echo_now("-----------------------------------------------");
            foreach ($array as $item_provider) {
                ...
                if ($response && isset($response->status) && $response->status == 200) {
                    foreach ($response->data as $item_rs) {
                        $log_buff = Instagram::where('orders_id', $item_rs->orders_id ?? $item_rs->id)->first();
                        if ($log_buff) {
