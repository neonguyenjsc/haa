...
class AutoUpdateCountIsRunMfb extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'update_count_is_run_facebook';
    protected $limit = 500;
            ...
            foreach ($array as $item_provider) {
                ...
                if ($response && isset($response->status) && $response->status == 200) {
                    foreach ($response->data as $item_rs) {
                        $log_buff = Facebook::where('orders_id', $item_rs->orders_id ?? $item_rs->id)->first();
                        if ($log_buff) {
