...
class AutoUpdateStarViewDVO extends Command
{
    use Lib;
     ...
     * @var string
     */
    protected $signature = 'update_start_view_dvo';
...
//        $this->sendError500ToTelegram("Bắt đầu udate view yt");
    ...
    public function dvo()
    {
        $list = Facebook::whereIn('package_name', ['facebook_comment_sv6'])->where('orders_id', '>', 0)->take(200)->orderBy('id', 'DESC')->pluck('orders_id')->toArray();
        $list = implode(",", $list);
        $data = [
            'orders' => $list,
            'action' => 'status',
...
//        $response = false;
        if ($response) {
            foreach ($response as $i => $res) {
                $item = Facebook::where('orders_id', $i)->orderBy('id', 'desc')->first();
                if (!isset($res->error)) {
                    if ($item->status && in_array($item->status, [2, 4])) {
                        continue;
                    }
                    echo_now(json_encode($res));
                    $item->start_like = intval($res->start_count) ?? 0;
                    $conlai = $res->remains;
        ...
        for ($z = 0; $z <= 10; $z++) {
            //$list = TikTok::where('id',326028)->whereIn('package_name', ['tiktok_view_s13', 'tiktok_view_s5', 'tiktok_view_s14', 'tiktok_like_v10', 'tiktok_follow_sv11'])->where('orders_id', '>', 0)->skip(200 * $z)->take(200)->orderBy('id', 'DESC')->pluck('orders_id')->toArray();
            $list = TikTok::whereIn('package_name', ['tiktok_view_s13','tiktok_like_v7', 'tiktok_view_s5', 'tiktok_view_s14', 'tiktok_like_v10', 'tiktok_follow_sv11'])->where('orders_id', '>', 0)->skip(200 * $z)->take(200)->orderBy('id', 'DESC')->pluck('orders_id')->toArray();
            $list = implode(",", $list);
            $data = [
                'orders' => $list,
                'action' => 'status',
...
//        exit();
        ...
        $list = Youtube::whereIn('package_name', ['youtube_comment_2', 'youtube_comment_3'])->where('orders_id', '>', 0)->where('status', '<>', 0)->whereColumn('quantity', '<', 'count_is_run')->orderBy('id', 'DESC')->pluck('orders_id')->take(200)->toArray();
        $list = implode(",", $list);
        $data = [
            'orders' => $list,
            'action' => 'status',
