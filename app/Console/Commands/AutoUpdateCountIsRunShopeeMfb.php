...
class AutoUpdateCountIsRunShopeeMfb extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'update_count_is_run_shopee';
    protected $limit = 500;
 ...
 | |_| | |_) | (_| | (_| | ||  __/  | (_| (_) | |_| | | | | |_   | \__ \  | |  | |_| | | | |
  \__,_| .__/ \__,_|\__,_|\__\___|   \___\___/ \__,_|_| |_|\__|  |_|___/  |_|   \__,_|_| |_|
       |_|
\n";
        $keyCacheAutoUpdateCountIsRun = build_key_cache(['keyCacheAutoUpdateCountIsRunShopeeMfb']);
...
//                ],
                'shopee' => [
                    'url' => 'https://api.mfb.vn/api/advertising/shopee/list?limit=' . $this->limit,
                    'provider' => 'shopee',
                ],
            ];
            echo_now("-----------------------------------------------");
            foreach ($array as $item_provider) {
                ...
                if ($response && isset($response->status) && $response->status == 200) {
                    foreach ($response->data as $item_rs) {
                        $log_buff = Shopee::where('orders_id', $item_rs->orders_id ?? $item_rs->id)->first();
                        if ($log_buff) {
