                                ...
                                <table class="table align-middle mb-0">
                                    ...
                                    @foreach($data as $i=>$item)
                                        <tr>
                                                        ...
                                                        <ul class="dropdown-menu" style="">
                                                            @if($item->allow_remove)
                                                                <li><a class="dropdown-item" style="cursor: pointer"
                                                                       onclick="removeOrder('{{$item->id}}','{{getUriOrder()}}/remove/{{$item->id}}','{{$item->price_per_remove}}')">Hủy
                                                                        đơn</a>
                                                                </li>
                                                            ...
                                                            @if($item->is_check_order)
                                                                <li><a class="dropdown-item" style="cursor: pointer"
                                                                       onclick="checkStatusOrder('{{$item->id}}','{{getUriOrder()}}/check-order/{{$item->id}}')">Kiểm tra số lượng chạy</a>
                                                                </li>
                                                            @endif
                                                                @if($item->run_order)
                                                                    <li><a class="dropdown-item" style="cursor: pointer"
                                                                           onclick="checkStatusOrder('{{$item->id}}','{{getUriOrder()}}/run-order/{{$item->id}}')">Tiếp
                                                                            tục đơn hàng</a>
                                                                    </li>
                                                                @endif
                                                        </ul>
                                            ...
                                            <td>{{$item->quantity}}</td>
                                            <td class="text-success">{{$item->count_is_run}}</td>
