...
@section('content')
                                ...
                                <div class="col-md-8">
                                    <form action="{{getUri()}}/buy" method="post" id="form_order">
                                        ...
                                        <div class="form-group">
                                            ...
                                            <input name="object_id" onchange="checkOutCoin()" id="object_id" type="text"
                                                   class="form-control rounded-10" required
                                                   placeholder="Nhập ID hoặc Link tùy theo gói">
                                            <input style="display: none" name="follower_count">
                                            <input style="display: none" name="linkshop">
                                            <input style="display: none" name="portrait">
                                            <input style="display: none" name="shopid">
    ...
    <script>
        ...
        function convertUidShopee() {
            loading();
            $.ajax({
                "url": "/api/shopee-follow/convert-uid",
                "method": "POST",
                "timeout": 0,
                "headers": {
                ...
                "data": JSON.stringify({
                    "link": $('input[name="object_id"]').val()
                }),
                success: function (data) {
                    stopLoading();
                    if (data.data.username) {
                        $('input[name="object_id"]').val(data.data.username);
                        $('input[name="follower_count"]').val(data.data.follower_count);
                        $('input[name="linkshop"]').val(data.data.linkshop);
                        $('input[name="portrait"]').val(data.data.portrait);
                        $('input[name="shopid"]').val(data.data.shopid);
                        $('input[name="username"]').val(data.data.username);
                        modalSuccess('Lấy thông tin thành công');
                    } else {
                        // modalError('Không thể get Id cho bạn. Vui lòng nhập id thủ công');
                    }
                },
                error: function (data) {
                    stopLoading();
